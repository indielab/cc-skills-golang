...
// MustInvoke panics on error — prefer this everywhere else
db := do.MustInvoke[Database](injector)
```

Inside a provider function, always use `do.MustInvoke` (or `MustInvokeAs`/`MustInvokeNamed`/`MustInvokeStruct`) rather than the error-returning variant. A provider already returns `(T, error)`, so propagating a dependency failure with `do.Invoke` costs an extra `if err != nil { return nil, err }` on every call. `do.MustInvoke` panics instead, and samber/do recovers that panic at the enclosing `Invoke` call and converts it back into a regular error — so the failure still surfaces as an error to the caller, just without the manual boilerplate in every provider.
...
6. Use `do.MustInvoke*` inside provider functions instead of `do.Invoke*` — samber/do turns the panic back into a returned error at the outer `Invoke` call, so you get the same error propagation without the boilerplate
